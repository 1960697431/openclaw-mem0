...
### Local Embeddings (Zero Dependencies)

Use `transformersjs` provider to run embeddings locally with ONNX models — no Ollama, no Python, no API keys:
...
"oss": {
  ...
  "vectorStore": {
    ...
    "config": {
      "dimension": 1024,
      "dbPath": "~/.openclaw/mem0-vectors.db"  // Fixed storage path
    }
  },
  "llm": { "provider": "openai", "config": { "model": "gpt-4o" } }
}
...
**Supported models:**
| Model | Dims | Size | Languages |
|-------|------|------|-----------|
| `onnx-community/Qwen3-Embedding-0.6B-ONNX` (recommended) | 1024 | ~700MB | 100+ |
| `Xenova/bge-small-en-v1.5` | 384 | ~130MB | English |
| `Xenova/multilingual-e5-large` | 1024 | ~2GB | 100 |

Models auto-download on first use. The plugin also includes a `JsonCleaningLLM` wrapper that automatically strips markdown code blocks from LLM responses — fixes Gemini and other models that return `\`\`\`json...\`\`\`` instead of raw JSON.
...
| `oss.embedder.provider` | `string` | `"openai"` | Embedding provider (`"openai"`, `"ollama"`, `"transformersjs"`) |
